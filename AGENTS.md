...
- Mind всегда первая, всегда развёрнута
...
#### MindPanel (несворачиваемая, всегда видима)

```typescript
@model("imai/MindPanel")
class MindPanel extends BasePanel {
  content: prop<string>("");
...
class NormalMode implements Mode {
  ...
  getContext() {
    return session.mind.renderForModel() + 
...
function renderForModel(session: Session): string {
  ...
  // 1. Mind — всегда первая, всегда полная
  parts.push(session.mind.renderForModel());
  
  // 2. Остальные панели по состоянию
  for (const panel of session.panels) {
    if (panel.id === "mind") continue;
...
- [ ] MindPanel (текст, агент управляет)
...
Builtin панели (Mind, Chat) — в коде системы или тоже файлы?
...
| Mind | Несворачиваемая панель с внутренним состоянием агента |
